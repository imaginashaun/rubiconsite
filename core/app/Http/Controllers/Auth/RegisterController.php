...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $page_title = "Journalist Sign Up";
        $info = json_decode(json_encode(getIpInfo()), true);
        $country_code = @implode(',', $info['code']);
        return view($this->activeTemplate . 'user.auth.journalist', compact('page_title','country_code'));
    }

    public function member()
    {
        $page_title = "Member Sign Up";
        $info = json_decode(json_encode(getIpInfo()), true);
        $country_code = @implode(',', $info['code']);
        return view($this->activeTemplate . 'user.auth.member', compact('page_title','country_code'));
    }

    public function userType()
    {
        $page_title = "Register";
        return view($this->activeTemplate . 'user.auth.user_type', compact('page_title'));
    }
    ...
    protected function validator(array $data)
    {
        $validate = Validator::make($data, [
            ...
            'user_type' => [Rule::in(["member","journalist"])],
    ...
    public function register(Request $request)
    {
        if(!$request->has('user_type')){
            $notify[] = ['error', "Invalid User Type"];
    ...
    public function registered()
    {
        ...
        if($user->user_type == 'member'){
            return redirect()->route('user.member.home');
        }
