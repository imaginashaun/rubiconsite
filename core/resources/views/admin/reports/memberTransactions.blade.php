                            ...
                            @forelse($transactions as $trx)
                                <tr>
                                    ...
                                    <td data-label="@lang('Username')">
                                        <a href="{{ route('admin.member.users.detail', $trx->user_id) }}">{{ @$trx->user->username }}</a>
                                    </td>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('admin.member.users.transactions'))
    ...
    @else
        <form action="{{ route('admin.report.member.transaction.search') }}" method="GET" class="form-inline float-sm-right bg--white">
            <div class="input-group has_append">
                <input type="text" name="search" class="form-control" placeholder="@lang('TRX / Username')" value="{{ $search ?? '' }}">
                <div class="input-group-append">
                    <button class="btn btn--primary" type="submit"><i class="fa fa-search"></i></button>
