                            ...
                            @forelse($items as $item)
                                <tr>
                                    ...
                                    <td data-label="@lang('Submitted By')">
                                        @if($item->user_id)
                                            @php
                                                $users = App\User::findOrFail($item->user_id)
                                            @endphp

                                            @if($users->user_type == "member")
                                                <a href="{{ route('admin.member.users.detail', $item->user_id) }}">{{ @$item->user->username }}</a>
                                            @else
                                                <a href="{{ route('admin.users.detail', $item->user_id) }}">{{ @$item->user->username }}</a>
                                            @endif
                                        @else
