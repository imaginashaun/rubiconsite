        ...
        <div class="col-lg-12">
            <div class="card b-radius--10 ">
                    ...
                    <div class="table-responsive--md  table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Username')"><a href="{{ route('admin.member.users.detail', $user->id) }}">{{ $user->username }}</a></td>
                                <td data-label="@lang('Email')">{{ $user->email }}</td>
                                <td data-label="@lang('Phone')">{{ $user->mobile }}</td>
                                <td data-label="@lang('Website')"><a href="{{ $user->website }}" target="_blank">{{ $user->website }}</a></td>
                                <td data-label="@lang('Joined At')">{{ showDateTime($user->created_at) }}</td>
                                <td data-label="@lang('Action')">
                                    <a href="{{ route('admin.member.users.detail', $user->id) }}" class="icon-btn" data-toggle="tooltip" title="" data-original-title="@lang('Details')">
                ...
                <div class="card-footer py-4">
                    {{ $member_users->links('admin.partials.paginate') }}
                </div>
            </div><!-- card end -->
        </div>
