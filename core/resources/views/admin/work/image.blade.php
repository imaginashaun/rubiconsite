        ...
        <div class="col-lg-12">
            <div class="card b-radius--10 ">
                        ...
                        @forelse($images as $value)
                            <tr>
                                <td data-label="@lang('Title')">
                                    <div class="user">
                                        <div class="thumb">
                                            <img src="{{ getImage('assets/images/workFile/'.$value->image)}}" alt="image">
                                        </div>
                                        <span class="name">{{ Str::words($value->title, 10) }}</span>
                                    </div>
                                </td>
                                ...
                                <td data-label="@lang('Action')">
                                  <a href="{{route('admin.work.image.detail', $value->id)}}" class="icon-btn btn--primary ml-2" data-toggle="tooltip" title="" data-original-title="Details">
                ...
                <div class="card-footer py-4">
                    {{ $images->links('admin.partials.paginate') }}
                </div>
            </div><!-- card end -->
        </div>
    </div>
...
<div id="deleteVideo" class="modal fade" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
       <div class="modal-content">
           ...
           <form action="{{route('admin.work.image.delete')}}" method="POST">
               @csrf
...
<div id="approvedVideo" class="modal fade" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
       <div class="modal-content">
           ...
           <form action="{{route('admin.work.image.approvedBy')}}" method="POST">
               @csrf
               <input type="hidden" name="id" value="">
               <div class="modal-body">
